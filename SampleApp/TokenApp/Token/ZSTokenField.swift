...
enum ZSTokenFieldState {
   case opened
   case closed
}


@objc protocol ZSTokenFieldDelegate : UITextFieldDelegate {
   func tokenFieldShouldChangeHeight(_ height: CGFloat)
   @objc optional func tokenFieldDidSelectToken(_ token: ZSToken)
   @objc optional func tokenFieldDidBeginEditing(_ tokenField: ZSTokenField)
   @objc optional func tokenFieldDidEndEditing(_ tokenField: ZSTokenField)
}


open class ZSTokenField: UITextField {
   ...
   fileprivate var _state: ZSTokenFieldState = .opened
   ...
   fileprivate var _direction: ZSTokenViewScrollDirection = .vertical {
      didSet {
   ...
   weak var parentView: ZSTokenWordView? {
      willSet (tokenView) {
         if (tokenView != nil) {
   ...
   weak var tokenFieldDelegate: ZSTokenFieldDelegate? {
      didSet {
   ...
   fileprivate func _setupTokenField() {
      ...
      addTarget(self, action: #selector(ZSTokenField.tokenFieldTextDidChange(_:)), for: UIControl.Event.editingChanged)
   }
   ...
   override open func draw(_ rect: CGRect) {
      ...
      // https://github.com/khawars/ZSTokenView/issues/11
   ...
   */
   func addTokenWithTitle(_ title: String) -> ZSToken? {
   ...
   - returns: ZSToken object
   */
   func addTokenWithTitle(_ title: String, tokenObject: AnyObject?) -> ZSToken? {
   ...
   - returns: ZSToken object
   */
   func addToken(_ token: ZSToken) -> ZSToken? {
      if (token.title.count == 0) {
         token.title = "Untitled"
      }
      
      if (!tokens.contains(token)) {
         token.addTarget(self, action: #selector(ZSTokenField.tokenTouchDown(_:)), for: .touchDown)
         token.addTarget(self, action: #selector(ZSTokenField.tokenTouchUpInside(_:)), for: .touchUpInside)
         tokens.append(token)
         _insertToken(token)
      }
   ...
   fileprivate func _insertToken(_ token: ZSToken, shouldLayout: Bool = true) {
   ...
   */
   ...
   - parameter token: ZSToken object
   */
   func deleteToken(_ token: ZSToken) {
      removeToken(token)
   }
   ...
   */
   func removeToken(_ token: ZSToken, removingAll: Bool = false) {
   ...
   */
   fileprivate func _layoutTokens() -> CGPoint {
      ...
      for token: ZSToken in tokens {
         let width = ZSUtils.getRect(token.title as NSString, width: bounds.size.width, font: _font!).size.width + ceil(_paddingX!*2+1)
   ...
   */
   ...
   fileprivate func _updateText() {
      ...
      case .closed:
         if tokens.count == 0 {
            ...
            let width = ZSUtils.widthOfString(title, font: font!)
            if width + _leftViewRect().width > bounds.width {
               text = "\(tokens[0]), \(tokens[1]) & \(tokens.count - 2) \(_descriptionText)"
            } else {
               text = title
            }
         }
   ...
   fileprivate func _updatePlaceHolderVisibility() {
      if tokens.count == 0 && (text == ZSTextEmpty || text!.isEmpty) {
   ...
   //
   func isSelectedToken(_ token: ZSToken) -> Bool {
      if token.isEqual(selectedToken) {
         return true
      }
      return false
   ...
   func selectToken(_ token: ZSToken) {
      if (token.sticky) {
         return
      }
      for token: ZSToken in tokens {
   ...
   @objc func tokenTouchDown(_ token: ZSToken) {
   ...
   @objc func tokenTouchUpInside(_ token: ZSToken) {
      selectToken(token)
   }
...
//__________________________________________________________________________________
//
extension ZSTokenField : UIScrollViewDelegate {
